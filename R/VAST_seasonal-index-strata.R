...
# remotes::install_github("james-thorson/VAST@main", INSTALL_opts="--no-staged-install")
...
ecomon_strata <- ecomon_strata %>%
  mutate(stock = case_when(strata %in% c(1380, 1390) ~ "EGOM",
                           strata %in% c(1090, 1100, 1230, 1240,
                                         1250, 1260, 1270, 1280,
                                         1370, 1400) ~ "WGOM",
                           strata %in% c(1130, 1140, 1150, 1160,
                                         1170, 1180, 1190, 1200,
                                         1210, 1220) ~ "GB",
                           strata %in% c(1010, 1020, 1050, 1060,
                                         1650, 1690, 1730, 1740) ~ "SNES",
                           strata %in% c(1290, 1300, 1360) ~ "WSS",
                           TRUE ~ NA_character_)) %>%
  filter(!is.na(stock))
...
vast_wrapper <- function(sp, n_x = 100) {
  ...
  ## Spring model -----
  working_dir <- here::here(sprintf("analysis/vast_seasonal_index_cod/%s_seasonal", sp))
  ...
  zoop_dat <- ecomon_strata %>%
    dplyr::filter(grepl(sprintf("^%s", sp) , spp),
                  stock %in% c("EGOM", "WGOM", "GB", "SNES", "WSS"),
  ...
  #   geom_point(data = zoop_dat %>% filter(abundance > 0), aes(color = stock, size = abundance), alpha = 0.5) +
  ...
  dummy_data = data.frame(
    ...
    lat = mean(zoop_dat[,'lat'], na.rm = TRUE),
    lon = mean(zoop_dat[,'lon'], na.rm = TRUE),
    areaswept_km2 = mean(zoop_dat[,'areaswept_km2'], na.rm = TRUE),
    abundance = 0,
    dummy = TRUE)
  ...
  strata.limits <- list("EGOM" = c(1380, 1390),
                        "WGOM" = c(1090, 1100, 1230, 1240,
                                   1250, 1260, 1270, 1280,
                                   1370, 1400),
                        "GB" = c(1130, 1140, 1150, 1160,
                                 1170, 1180, 1190, 1200,
                                 1210, 1220),
                        "SNES" = c(1010, 1020, 1050, 1060,
                                   1650, 1690, 1730, 1740),
                        "WSS" = c(1290, 1300, 1360))
