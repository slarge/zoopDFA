...
working_dir <- here::here("analysis/vast_seasonal_EOF3_dual_310/")

results <- readRDS(here::here("analysis/vast_seasonal_EOF3_dual_310/results.rds"))

fit <- readRDS(here::here("analysis/vast_seasonal_EOF3_dual_310/fit.rds"))
...
yearseason_set <- expand.grid(year_season = paste0(rep(1992:2023, each = 4), "_",
...
index <- read.csv(file = here::here("analysis/vast_seasonal_EOF3_dual_310/index.csv")) %>%
...
index_plot <- ggplot(index, aes(x = time, y = value/1000000000, ymin = value_lo/1000000000, ymax = value_up/1000000000,
                                color = epu, fill = epu)) +
...
E = list(
  ...
  "u" = fit$Report$Epsiloninput1_gff[, 1,]
)

## Construct the seasonal variable
t_series <- expand_grid(year = 1992:2023,
...
cca_res <- stats::cancor(EOFset1, t_sig$signal_vec, xcenter = TRUE) # perform CCA
...
factor_cluster_pca_plot <- ggplot(factor_time %>% filter(name != "cca"), aes(x = time, y = value, color = season)) +
  geom_segment(aes(xend = time, yend = 0), na.rm = FALSE, linewidth = 0.5) +
  geom_point(na.rm = FALSE, size = 0.5) +
  geom_text(data = labels_pca, aes(label = label), x = 1992, y = 1.8, hjust = 0, vjust = 0, inherit.aes = FALSE) +
...
factor_cluster_cca_plot <- ggplot(factor_time %>% filter(name == "cca"), aes(x = time, y = value, color = season)) +
  geom_segment(aes(xend = time, yend = 0), na.rm = FALSE, linewidth = 0.5) +
  geom_point(na.rm = FALSE, size = 0.5) +
  geom_text(data = labels_cca, aes(label = label), x = 1992, y = 0.25, hjust = 0, vjust = 0, inherit.aes = FALSE) +
  geom_hline(yintercept = 0) +
  # geom_path(data = t_sig, aes(x = year)) +
  # geom_line() +
  labs(x = "", y = "Factor Loadings", color = "Season") +
  scale_color_brewer(palette = "Set1") +
  facet_grid(group~name) +
...
ggsave(filename = here::here("analysis/vast_seasonal_EOF3_dual_310/cluster_loadings_pca.jpeg"), cluster_loadings_pca, bg = "white",
       width = 170, height = 191, units = "mm", device = jpeg, dpi = 500)
ggsave(filename = here::here("analysis/vast_seasonal_EOF3_dual_310/cluster_loadings_cca.jpeg"), cluster_loadings_cca, bg = "white",
       width = 170, height = 191, units = "mm", device = jpeg, dpi = 500)
...
ggsave(filename = here::here("analysis/vast_seasonal_EOF3_dual_310/cluster_loadings_spp.jpeg"), tree_plot_spp, bg = "white",
       width = 170, units = "mm", device = jpeg, dpi = 500)


epsilon1_loadings_se <- data.frame(results$Factors$Rotated_loadings_SE$Epsilon1) %>%
...
ggsave(filename = here::here("analysis/vast_seasonal_EOF3_dual_310/factor_loadings_spp.jpeg"), factor_taxa, bg = "white",
       width = 170, units = "mm", device = jpeg, dpi = 500)
...
ccavar1_x <- data.frame(season_n = 1:4) %>%
  mutate(dat = purrr::map(.x = season_n, ~ data.frame(fit$spatial_list$latlon_g,
                                                      "factor" = "cca",
                                                      "epsilontime" = (as.matrix(fit$Report$Epsiloninput1_gff[,.x,]) %*% cca_res$xcoef[,1]))),
...
epsilontime_1 <- data.frame(results$Factors$Rotated_projected_factors$EpsilonTime1,
...
epsilontime_plot <- ggplot() +
  geom_sf(data = epsilontime_1, aes(color = epsilontime), alpha = 0.5) +
...
epsilontime_cca_plot <- ggplot() +
  geom_sf(data = ccavar1_x, aes(color = epsilontime), alpha = 0.5) +
  geom_sf(data = ne_countries, color = "grey40", size = 0.05) +
  geom_sf(data = ne_states, color = "grey40", size = 0.05) +
  facet_grid(season~.) +
  coord_sf(crs = crs, xlim = xlims, ylim = ylims) +
...
ggsave(filename = here::here("analysis/vast_seasonal_EOF3_dual_310/epsilontime_cca_season.jpeg"), epsilontime_plot, bg = "white",
       width = 85, units = "mm", device = jpeg, dpi = 500)
